...
It is easy to scale up recommendation or vector similarity search with `hyperscale` after training a model using any popular library. Simply extract the embedding vectors and feed them to `hyperscale`. Some example code snippets are given below.
...
The tree is built `k` times to generate a forest of hyperplanes. This is controlled by the `n_trees` parameter when building the vector index. `k` can be tuned to your needs based on the trade-off between precision and performance. In practice, `k` should probably be around the same order of vector dimensionality.
